...
typedef struct {
    ...
    uint8_t *correct;
} chacha20_cipher_test_t;

static const chacha20_cipher_test_t TESTS[] = {
    {
        .key_bytes = {
...
#define NUM_CASES (sizeof(TESTS) / sizeof(chacha20_cipher_test_t))

int main() {
    bool pass;
    unsigned int tests_passed = 0;

    printf("RUNNING CIPHER TESTS ...\nDIFFERENCE LINE:\n[.] - equal bytes, [*] - different bytes\n\n");

    for (unsigned i = 0; i < NUM_CASES; i++) {
        printf("[TEST] #%d\n", i+1);
        pass = true;

        uint8_t msg[TESTS[i].msg_len];
        for (unsigned b = 0; b < TESTS[i].msg_len; b++) msg[b] = TESTS[i].msg[b];

        chacha20_cipher(TESTS[i].key_bytes, TESTS[i].nonce_bytes, TESTS[i].counter, msg, TESTS[i].msg_len);

        for (unsigned j = 0; j < TESTS[i].msg_len; j++) {
            if (msg[j] == TESTS[i].correct[j]) {
                putc('.', stdout);
            } else {
                pass = false;
                putc('*', stdout);
